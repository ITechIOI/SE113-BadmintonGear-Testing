...
export default function AdminProductItem({ product, onCheck, onDelete }) {
    ...
    const status = product.quantity === 0 ? 'Out of Stock' : (product.quantity > 10 ? 'In Stock' : "Low Stock");
    const statusColor = product.quantity === 0 ? 'text-[#F04438]' : (product.quantity > 10 ? 'text-[#0D894F]' : "text-[#ff8200]");
    const bgColor = product.quantity === 0 ? 'bg-[#FEEDEC]' : (product.quantity > 10 ? 'bg-[#E7F4EE]' : "bg-[#FDF1E8]");
    return (
        <tr className='bg-white shadow-md border-b border-[#F0F1F3]'>
            ...
            <td className='py-4'>
                <div className='flex  items-center gap-2'>
                    <Image src={product.imageUrl ? getLinkImage(product.imageUrl) : '/images/placeholder.png'} alt={product.name} width={50} height={50} className='object-contain rounded-lg' />
                    <label className='font-medium text-black'>{product.name}</label>
                </div>
            </td>
            <td className='text-[#ff8200]'>{product.brand}</td>
            <td>{product.category.name}</td>
            <td>{product.quantity}</td>
            <td>${product.price}</td>
            <td className='flex items-center justify-center py-4'>
                <div className={statusColor + " " + bgColor + " px-2 py-1 rounded-full w-fit "}>{status}</div>
            </td>
            ...
            <td >
                <div className='flex justify-center items-center gap-3'>
                    <button onClick={() => router.push(`/admin/productdetail?id=${product.id}&mode=view`)}>
                    ...
                    <button onClick={() => router.push(`/admin/productdetail?id=${product.id}&mode=edit`)}>
                        <svg width="20" height="20" viewBox="0 0 20 20" fill="none" xmlns="http://www.w3.org/2000/svg">
