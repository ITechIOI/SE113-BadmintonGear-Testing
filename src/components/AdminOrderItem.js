...
export default function AdminOrderItem({ order, button }) {
  ...
  //                         productImage: product ? product.imageUrl : '/images/placeholder.png',
  //                     };
  //                 })
  //             );
  ...
  //     }
  // }
  // useEffect(() => { fetchOrderDetails(); }, [order]);
  return (
    <tr className="bg-white shadow-md border-b border-[#F0F1F3] py-4">
      {/* <td>
                <input
                    type='checkbox'
      ...
      <td className="text-[#ff8200] font-semibold ">#{order.id}</td>
      {/* <td className='py-4'>
                <div className="flex items-center gap-3">
                    Hiển thị sản phẩm đầu tiên
                    <Image
                        src={getLinkImage(details && details[0] ? details[0].productImage : "/images/placeholder.png")}
                        alt={details && details[0] ? details[0].productName : ""}
                        className="rounded-lg"
                        width={50}
                        height={50}
                    />
                    <div>
                        <div className="font-medium">{details && details[0] ? details[0].productName : ""}</div>
                        <div className="text-sm text-gray-500">
                            ${details && details[0] ? details[0].price : ""} x {details && details[0] ? details[0].quantity : ""}
                        </div>
      ...
      <td>
        <div className="py-4">
          <div className="font-medium">{order ? order.address : ""}</div>
