...
export default function OrderPage() {
  const [order, setOrder] = useState([
    { id: "1", total: 345, state: "Completed" }
  ]); // Khởi tạo state cho danh sách dơn hàng
  const [user, setUser] = useState(null);
  useEffect(() => {
    const fetchOrders = async () => {
      try {
        const storedUser = Cookies.get("user"); // lấy thông tin user từ cookie
        ...
        // Gọi API lấy orders theo userId
        const orders = await getOrderByUserId(userId);

        if (orders && Array.isArray(orders)) {
          const orderFormat = orders.map((ord) => ({
            id: ord.id,
        ...
        if (storedUser) {
          setUser(JSON.parse(storedUser)); // nhớ parse JSON thay vì set raw string
        }
      } catch (error) {
