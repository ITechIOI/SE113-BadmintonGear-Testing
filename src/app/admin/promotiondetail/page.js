...
import { getPromotionById, updatePromotion, addPromotion } from '@/api/promotionApi';
...
export default function PromotionDetail(id) {
    ...
    function formatDateToJava(dateStr) {
    ...
    const handleInputChange = () => {
        ...
        if (new Date(end) < new Date() || new Date(start) > new Date()) {
            ...
            if (quantity > 0) {
                ...
                status = 'out of turn'; // Out of turn
            } else {
    ...
    const handleCancel = () => {
        if (mode === 'add') {
            ...
            document.getElementById('quantity').value = promotion.count;
            document.getElementById('value').value = promotion.percent;
            document.getElementById('start').value = new Date(promotion.startTime).toISOString().slice(0, 16);
            document.getElementById('end').value = new Date(promotion.endTime).toISOString().slice(0, 16);
            document.getElementById('min').value = promotion.minOrderValue;
    ...
    const updatePromotionDetails = async () => {
        if (!inputPromotion || !inputPromotion.code || !inputPromotion.count || !inputPromotion.percent || !inputPromotion.startTime || !inputPromotion.endTime || !inputPromotion.minOrderValue) {
            ...
            <div className='flex justify-between items-end'>
                <div>
                    ...
                    <div id="roadmap" className="flex items-center mt-2">
                        ...
                        <a className="text-[#ff8200]" href="/admin/promotion">Promotion List</a>
                        <label className="ml-3 mr-3">
                            <svg width="18" height="18" viewBox="0 0 18 18" fill="none" xmlns="http://www.w3.org/2000/svg">
                                <path fillRule="evenodd" clipRule="evenodd" d="M6.59467 3.96967C6.30178 4.26256 6.30178 4.73744 6.59467 5.03033L10.5643 9L6.59467 12.9697C6.30178 13.2626 6.30178 13.7374 6.59467 14.0303C6.88756 14.3232 7.36244 14.3232 7.65533 14.0303L12.4205 9.26516C12.5669 9.11872 12.5669 8.88128 12.4205 8.73484L7.65533 3.96967C7.36244 3.67678 6.88756 3.67678 6.59467 3.96967Z" fill="#A3A9B6" />
                            </svg>
                        </label>
                        <a className="text-[#667085]" href={promotionId ? `/admin/promotiondetail?id=${promotionId}&&mode=${mode}` : '/admin/promotiondetail?mode=add'}>Promotion Details</a>
                    </div>
                ...
                {mode !== 'view' && (
                    <div className='flex gap-3'>
                        ...
                        <button className='bg-[#ff8200] text-white px-4 py-2 rounded-md flex gap-2 items-center disabled:opacity-65'
                            disabled={!inputPromotion || !inputPromotion.code || !inputPromotion.count || !inputPromotion.percent || !inputPromotion.startTime
                                || !inputPromotion.endTime || !inputPromotion.minOrderValue || (promotion && inputPromotion.code === promotion?.code && inputPromotion.description === promotion?.description &&
                                    inputPromotion.count === promotion?.count && inputPromotion.percent === promotion?.percent && inputPromotion.startTime === promotion?.startTime
                                    && inputPromotion.endTime === promotion?.endTime && inputPromotion.minOrderValue === promotion?.minOrderValue)}
                            onClick={handleSave}>
                            <svg width="20" height="20" viewBox="0 0 20 20" fill="none" xmlns="http://www.w3.org/2000/svg">
                        ...
                        {mode === 'edit' && promotion && promotion.status === "available" && (
                    ...
                    <div className='bg-white shadow-md rounded-lg p-5'>
                        ...
                        {mode !== 'add' && promotion && (
                            <div className='mt-2 gap-2 flex items-center'>
                                ...
                                <div className={`py-1 px-2 rounded-full ${promotion.status === "available" ? 'text-[#0D894F] bg-[#E7F4EE]' : 'text-[#F04438] bg-[#FEEDEC]'}`}>
                    ...
                    <div className='bg-white shadow-md rounded-lg p-5 mt-5'>
                        ...
                        <div className='mt-2 gap-1'>
                            ...
                            <input id='value' type="number" className='border border-[#E0E2E7] bg-[#F9F9FC] rounded-md w-full px-3 py-2  outline-none'
                                ...
                                value={
                                    mode === 'add'
                                        ? (inputPromotion?.percent ?? "")
                                        : (promotion?.percent ?? "")}
                                onChange={handleInputChange}
                                disabled={mode === 'view'} />
                        </div>
                        <div className='mt-2 gap-1'>
                            <label className='text-sm font-medium ml-2'>Min Value of Order (%)</label>
                            <input id='min' type="number" className='border border-[#E0E2E7] bg-[#F9F9FC] rounded-md w-full px-3 py-2  outline-none'
                                placeholder='Type promotion value. . .' min={0}
                                value={
                                    mode === 'add'
                                        ? (inputPromotion?.minOrderValue ?? "")
                        ...
                        <div className='flex justify-start gap-10 items-center w-full'>
                            <div className='mt-2 gap-1 flex flex-col w-full'>
                                ...
                                <input id='start' type="datetime-local" className='border border-[#E0E2E7] bg-[#F9F9FC] rounded-md w-full px-3 py-2 outline-none'
                                    ...
                                    defaultValue={promotion && promotion.startTime
                                        ? new Date(promotion.startTime).toISOString().slice(0, 16)
                            ...
                            <div className='mt-2 gap-1 flex flex-col w-full'>
                                ...
                                <input id='end' type="datetime-local" className='border border-[#E0E2E7] bg-[#F9F9FC] rounded-md w-full px-3 py-2  outline-none'
                                    ...
                                    defaultValue={promotion && promotion.endTime
                                        ? new Date(promotion.endTime).toISOString().slice(0, 16)
                                        : new Date().toISOString().slice(0, 16)}
                                    onChange={handleInputChange}
                                    disabled={mode === 'view'} />
                            </div>
