...
import { getAllPromotions, deletePromotionById } from '@/api/promotionApi';
...
export default function PromotionPage() {
    ...
    const handleSelectAll = () => {
        ...
        setDisplayedPromotions(promotions.map(promotion => ({ ...promotion, isChecked: newCheckedState })));
    };
    ...
    const fetchPromotions = async () => {
        ...
        if (data) {
            ...
            setPromotions(data.map(promotion => ({ ...promotion, isChecked: false })));
            setDisplayedPromotions(data.map(promotion => ({ ...promotion, isChecked: false })));
        } else {
    ...
    const handleFilterChange = (e) => {
        ...
        if (value === "") {
            ...
            const filteredPromotions = promotions.filter(promotion => promotion.status === value);
            setDisplayedPromotions(filteredPromotions);
        }
    };

    const handleDeleteCheckedPromotions = () => {
        const checkedPromotions = promotions.filter(promotion => promotion.isChecked);
        if (checkedPromotions.length === 0) {
            alert("No promotions selected for deletion.");
            return;
        }
        if (window.confirm(`Are you sure you want to delete ${checkedPromotions.length} promotions?`)) {
        ...
        <div className='font-inter'>
            <div className="flex justify-between items-end">
                ...
                <div className='flex gap-3'>
                    ...
                    <button className='bg-[#ff8200] text-white px-4 py-2 rounded-md flex gap-2 items-center'
                        onClick={() => { router.push('/admin/promotiondetail?mode=add') }}>
                        <svg width="20" height="20" viewBox="0 0 20 20" fill="none" xmlns="http://www.w3.org/2000/svg">
                    ...
                    {isCheck && (
                        <button className='bg-[#ff8200] text-white px-4 py-2 rounded-md flex gap-2 items-center' onClick={handleDeleteCheckedPromotions}>
                            <svg width="20" height="20" viewBox="0 0 20 20" fill="none" xmlns="http://www.w3.org/2000/svg">
            ...
            <div className='mt-5 flex justify-between items-center'>
                <div className='flex gap-1 items-center bg-white rounded-md px-4 border border-[#E0E2E7]'>
                    ...
                    <input type='text' placeholder='Search promotion...' className='px-2 py-2 outline-none' onChange={handleSearchChange} />
                </div>
                ...
                {showFilter && (
                    <div ref={filterRef} className='absolute top-16 right-5 bg-white shadow-lg rounded-md p-4 border border-[#E0E2E7]'>
                        <h3 className='font-semibold mb-2'>Filter Promotions</h3>
                        <select className='w-full p-2 border border-[#E0E2E7] rounded-md' onChange={handleFilterChange} defaultValue="">
                            <option value="">All Promotions</option>
                            <option value="available">Available Promotions</option>
                            <option value="disable">Disable Promotions</option>
                            <option value="expired">Expired Promotions</option>
                            <option value="out of turn">Out of Turn Promotions</option>
                        </select>
                    </div>
                )}
            </div>
                ...
                <table className='w-full py-2 rounded-md overflow-hidden '>
                    <thead className='bg-[#F9F9FC] font-medium border-b border-[#F0F1F3]'>
                        <tr className='text-center text-[#344054] font-semibold rounded-md'>
                            ...
                            <th className='py-2 px-4'>Min Order Value</th>
