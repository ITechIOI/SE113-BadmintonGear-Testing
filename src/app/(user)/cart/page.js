...
export default function Cart() {
    ...
    if (cartItems) {
      setItems(
        cartItems.map((item) => {
          ...
          console.log("Product info for cart item:", productInfo.price);
  ...
  const fetchData = async () => {
    ...
    console.log("Fetched products:", await fetchCartItems(productsList || []));
  };
        ...
        <div className="flex justify-between items-start mt-5">
          <div className="flex w-1/2">
            <input
              ...
              className="border border-gray-500 rounded-xs px-4 py-3 w-3/5"
              onChange={(e) => {
                setCouponCode(e.target.value);
              }}
            />
            <button
              className="bg-[#FF8200] text-white rounded-xs px-10 py-3 ml-5 cursor-pointer disabled:cursor-not-allowed"
              disabled={couponCode === ""}
          ...
          <div className="w-1/3 border-2 rounded-md px-10 py-5 flex flex-col">
            <label className="w-full text-lg font-medium mb-5 text-center">
              Cart Total
            </label>
            ...
            {promotion > 0 && (
              <div className="border-b border-gray-500 flex justify-between pb-3 mb-3">
                ...
                <p className="text-right">
                  - {Number(promotion).toLocaleString()} %
                </p>
              </div>
            )}
            <div className="flex justify-between mb-3">
              ...
              <p className="text-right">
                {Number(
                  subtotal + shipping - (promotion * subtotal) / 100
                ).toLocaleString()}{" "}
                $
              </p>
            </div>
            <button
              className="bg-[#FF8200] text-white rounded-xs px-10 py-3 mt-5 w-fit mx-auto"
              onClick={() => {
                const checkedItems = items.filter((item) => item.isChecked);
