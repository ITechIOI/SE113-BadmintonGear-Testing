const getAllUsers = async (page = 0, limit = 10) => {
  try {
    const url = `${process.env.NEXT_PUBLIC_API_URL}/users/users/all?page=${page}&limit=${limit}`;
    ...
    // Trả về nguyên object phân trang từ backend
    return {
      content: data?.data?.content || [],
      totalPages: data?.data?.totalPages || 1,
      totalElements: data?.data?.totalElements || 0,
      pageSize: data?.data?.size || limit,
