...
export default function AdminPromotionItem({ promotion, onCheck, onDelete }) {
    ...
    return (
        <tr className='bg-white shadow-md border-b border-[#F0F1F3]'>
            ...
            <td className='text-[#ff8200]'>{promotion.id}</td>
            ...
            <td className='flex justify-center items-center py-4'>
                <div className={`w-fit px-2 py-1 rounded-full ${promotion.status === "available" ? "text-[#0D894F] bg-[#E7F4EE]" : (promotion.status === "Out of turn" ? "text-[#E46A11] bg-[#FDF1E8]" : "text-[#F04438] bg-[#FEEDEC]")}`}>
                    {promotion.status}
                </div>
            </td>
            <td >
                <div className='flex  items-center gap-3 py-4 justify-center'>
                    <button onClick={() => router.push('/admin/promotiondetail?id=' + promotion.id + '&mode=view')}>
                    ...
                    <button onClick={() => router.push('/admin/promotiondetail?id=' + promotion.id + '&mode=edit')}>
                        <svg width="20" height="20" viewBox="0 0 20 20" fill="none" xmlns="http://www.w3.org/2000/svg">
